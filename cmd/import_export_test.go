...
func TestImportSubcommandFlagOwnership(t *testing.T) {
	...
	for _, importCommand := range []*cobra.Command{importFileCmd, importBrowserCmd, importBookmarksCmd, importLinkdingCmd, importLinkwardenCmd, importKarakeepCmd, importRaindropCmd, importReadeckCmd, importShaarliCmd, importWallabagCmd} {
	...
	for _, name := range []string{"backend", "backend-option", "proxy", "header", "cookie"} {
		for _, importCommand := range []*cobra.Command{importBrowserCmd, importBookmarksCmd, importLinkdingCmd, importLinkwardenCmd, importKarakeepCmd, importRaindropCmd, importReadeckCmd, importShaarliCmd} {
			if importCommand.Flags().Lookup(name) == nil {
				t.Errorf("import %s is missing --%s", importCommand.Name(), name)
			}
		}
